...
class DraftRepository {
  ...
  Future<DraftRequest?> load(String sub, String uuid) async {
    final row = await (db.select(db.localDrafts)
          ..where((t) => t.clientUuid.equals(uuid) & t.userSub.equals(sub)))
        .getSingleOrNull();
    return row == null ? null : _assemble(row);
  }

  Future<DraftRequest> _assemble(LocalDraft row) async {
    final lines = await (db.select(db.localLines)
  ...
  Future<void> save(String sub, DraftRequest d, {DateTime? now}) => db.transaction(() async {
        ...
        final existing = await (db.select(db.localDrafts)..where((t) => t.clientUuid.equals(d.clientUuid))).getSingleOrNull();
        if (existing != null && existing.userSub != sub) {
          throw StateError('Draft milik pengguna lain');
        }
        await db.into(db.localDrafts).insertOnConflictUpdate(LocalDraftsCompanion.insert(
        ...
        for (final l in d.lines) {
          await db.into(db.localLines).insert(LocalLinesCompanion.insert(
                clientUuid: l.clientUuid,
          ...
          for (final r in l.receipts) {
            await db.into(db.localReceipts).insert(LocalReceiptsCompanion.insert(
                  clientUuid: r.clientUuid,
  ...
  Future<void> setSyncState(
    ...
    bool clearError = false,
      ...
      (db.update(db.localDrafts)..where((t) => t.clientUuid.equals(uuid))).write(LocalDraftsCompanion(
  ...
  Future<void> setServerReceiptId(String receiptUuid, int id) =>
      (db.update(db.localReceipts)..where((t) => t.clientUuid.equals(receiptUuid)))
          .write(LocalReceiptsCompanion(serverReceiptId: Value(id)));

  Future<void> softDelete(String sub, String uuid) =>
      (db.update(db.localDrafts)..where((t) => t.clientUuid.equals(uuid) & t.userSub.equals(sub)))
          .write(const LocalDraftsCompanion(deleted: Value(true)));
  ...
  Future<void> addMedia(String sub, {required String uuid, required String kind, required Uint8List bytes, required String sha256, String mime = 'image/jpeg'}) async {
    if (await queueBytes(sub) + bytes.length > maxQueueBytes) throw const QueueFullException();
    await db.into(db.mediaBlobs).insertOnConflictUpdate(MediaBlobsCompanion.insert(
          clientUuid: uuid,
          userSub: sub,
          kind: kind,
          mimeType: Value(mime),
  ...
  Future<void> markMediaUploaded(String uuid, {String? serverMediaId}) =>
      (db.update(db.mediaBlobs)..where((t) => t.clientUuid.equals(uuid))).write(MediaBlobsCompanion(
