...
class _LoginScreenState extends ConsumerState<LoginScreen> {
  ...
  Widget build(BuildContext context) {
    ...
    final notice = auth is AuthSignedOut
        ? (auth.message == 'session_ended' ? t.sessionEnded : auth.message)
        : null;
    return Scaffold(
      body: SafeArea(
        child: Padding(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            ...
            if (notice != null)
              Card(
                ...
                child: Padding(padding: const EdgeInsets.all(12), child: Text(notice, key: const Key('login-notice'))),
              ),
            if (_error != null)
              Padding(
            ...
            if (!online) Padding(padding: const EdgeInsets.only(top: 8), child: Text(t.loginNeedsInternet, textAlign: TextAlign.center)),
          ]),
        ),
      ),
    );
