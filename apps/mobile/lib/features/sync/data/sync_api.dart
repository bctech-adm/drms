...
class SyncApi {
  ...
  Future<void> putMedia(String clientUuid, Uint8List bytes, {required String kind, required String sha256, String mime = 'image/jpeg'}) =>
      client.run(
        (d) => d.put<dynamic>(
          '/sync/media/$clientUuid',
          data: FormData.fromMap({
            'file': MultipartFile.fromBytes(bytes,
                filename: '$clientUuid.${mime == 'image/png' ? 'png' : 'jpg'}', contentType: DioMediaType.parse(mime)),
            'kind': kind,
            'sha256': sha256,
          }),
        ),
