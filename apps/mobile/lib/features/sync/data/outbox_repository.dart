...
class OutboxRepository {
    ...
    DateTime? now,
      ...
      db.transaction(() async {
        ...
        final pending = await (db.select(db.outbox)
            ...
            .getSingleOrNull();
        if (pending != null) {
          await (db.update(db.outbox)..where((t) => t.opUuid.equals(pending.opUuid))).write(OutboxCompanion(
  ...
  Future<List<OutboxData>> due(String sub, DateTime now) => (db.select(db.outbox)
        ..where((t) =>
            t.userSub.equals(sub) &
            t.status.equals('pending') &
  ...
  Future<List<OutboxData>> all(String sub) => (db.select(db.outbox)
        ..where((t) => t.userSub.equals(sub))
        ..orderBy([(t) => OrderingTerm.desc(t.createdAt)]))
      .get();

  Future<OutboxData?> pendingForTarget(String sub, String target) => (db.select(db.outbox)
  ...
  Future<void> setStatus(String opUuid, String status, {String? code, String? message}) =>
      (db.update(db.outbox)..where((t) => t.opUuid.equals(opUuid))).write(OutboxCompanion(
        status: Value(status),
  ...
  Future<void> retryFailed(String sub) => (db.update(db.outbox)
        ..where((t) => t.userSub.equals(sub) & t.status.equals('failed')))
      .write(const OutboxCompanion(status: Value('pending'), attempts: Value(0), nextAttemptAt: Value(null)));

  /// Pending ops of a draft that was submitted online are obsolete.
  Future<void> supersede(String sub, String target) => (db.update(db.outbox)
        ..where((t) => t.userSub.equals(sub) & t.targetUuid.equals(target) & t.status.equals('pending')))
  ...
  Stream<OutboxCounts> watchCounts(String sub) {
    ...
    return q.watch().map((rows) => OutboxCounts(
